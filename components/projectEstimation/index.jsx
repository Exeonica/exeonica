          ...
          <div className="mx-[454px] flex flex-col">
            ...
            {option.type === "radio" &&
              option.choices.map((choice, index) => (
                <div className="mb-7 rounded-[16px] border border-color-1 px-[24px] py-[30px] ">
                  <div key={index} className="flex justify-between">
                    <p >{choice}</p>
                    <input type="radio" name={`option${option.id}`} value={choice}  className={"accent-primary"} />
                  </div>
                </div>
              ))}
            {option.type === "checkbox" &&
              option.choices.map((choice, index) => (
                <div className="mb-7 rounded-[16px] border border-color-1 px-[24px] py-[30px]">
                  <div key={index} className="flex justify-between">
                    <p>{choice}</p>
                    <input type="checkbox" name={`option${option.id}`} value={choice} color="bg-primary" className={"accent-primary"} />
                  </div>
                </div>
              ))}
            ...
            {option.type === "radioWithIcon" &&
              option.choices.map((choice, index) => (
                <div key={index} className="mb-7 items-center justify-between rounded-[16px] border border-color-1 px-[24px] py-[27px]">
                  <div key={index} className="flex w-full justify-between">
                    <div className="flex items-center">
                        {choice.label==="Both"? null:
                      <Image src={choice.icon } alt={""} className="h-[36px] w-[36px] object-contain" />
                        }
                      <p className="ml-[16px]">{choice.label}</p>
                    </div>
                    <input type="radio" name={`option${option.id}`} value={choice.label} className={"accent-primary"} />
                  </div>
                </div>
              ))}
            {option.type === "checkBoxWithIcon" &&
              option.choices.map((choice, index) => (
                <div key={index} className="mb-7 items-center justify-between rounded-[16px] border border-color-1 px-[24px] py-[27px]">
                  <div key={index} className="flex w-full justify-between">
                    <div className="flex items-center">
                    {choice.label==="Others"? null:
                      <Image src={choice.icon } alt={""} className="h-[36px] w-[36px] object-contain" />
                        }
                      <p className="ml-[16px]">{choice.label}</p>
                    </div>
                    <input type="checkbox" name={`option${option.id}`} value={choice.label} className={"accent-primary"} />
                  </div>
                </div>
              ))}
            {option.type === "textarea" &&
                  option.placeholders.map((item, index) => (
                      <textarea key={index} placeholder={item.placeholders} className="mb-4 mt-2 w-full rounded border p-2" rows="2" />
                  ),
              )}
          </div>
        </>
    ...
    <div className="mb-[56px] flex flex-1 justify-center">
      <div className="mt-[36px] flex flex-1 flex-col items-center justify-center">
        ...
        <div className="mt-4 flex items-center justify-between">
          ...
          <Button classes="px-[73px] py-[12px] rounded-[8px]" 
           onClick={() => {
