    ...
    <div className="flex flex-col bg-border sm:justify-center md:flex-row md:py-[80px]">
      ...
      <div className="flex flex-1 flex-col items-center py-[80px] md:justify-center">
        <div className="sm:w-12/3 md:w-1/4">
          <Button variant="outline" size="full" className="font-pangeaAfrikanTrial mx-1 mb-[8px] mt-12 rounded-3xl">
            {strings["aboutUs"]}
          </Button>
        </div>
        ...
        <div className="mt-0 flex flex-col px-4 sm:px-6 lg:px-8">
          {" "}
          {/* Adjust padding */}
          <h1 className="font-pangeaAfrikanTrial leading-1 mb-[36px] mt-[8px] pb-[36] text-center text-[28px] font-semibold text-title sm:text-2xl md:text-[40px]">
        ...
        <div className="justify-cente mx-[37px] flex w-full flex-col items-center md:mx-[136px] md:w-9/12 md:flex-row md:justify-between">
          {images.map((image, index) => (
            <Image
              ...
              src={image.src}
              alt={image.alt}
              className={`m-2 h-[120px] w-[108.22px] object-contain ${image.classes || ""}`} // Add object-cover to maintain aspect ratio
            />
          ))}
        </div>
      </div>
